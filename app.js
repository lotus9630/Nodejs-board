...
app.get("/", (req, res) => {
  let title_list = "<ul>";
  connection.query(
    `select title from writing_list`,
    function (error, results, fields) {
      if (error) {
        ...
        results.forEach((element) => {
          title_list =
            title_list +
            `<li><a href="/page/${element.title}">${element.title}</a></li>`;
        });
...
app.post("/create", (req, res) => {
  ...
  connection.query(
    `INSERT INTO writing_list(title, description, writer) 
    VALUES ("${req.body.title}", "${req.body.description}", "${req.session.nickname}");`,
    function (error, results, fields) {
      console.log(results);
      if (error) {
        throw error;
...
app.get("/page/:title", (req, res) => {
  connection.query(
    `select description from writing_list where title="${req.params.title}" `,
    function (error, results, fields) {
      res.send(`
...
app.post("/update/:title", (req, res) => {
  ...
  connection.query(
    `select writer from writing_list where title = "${title}"`,
    function (error, results, fields) {
...
app.put("/update/:title", (req, res) => {
  connection.query(
    `update writing_list set title="${req.body.new_title}", description="${req.body.new_description}" 
    where title="${req.body.old_title}" and description="${req.body.old_description}" and writer="${req.session.nickname}"`,
    function (error, results, fields) {
      res.redirect(`/`);
    }
  );
  ...
  connection.query(
    ...
    function (error, results, fields) {
      ...
      if (nickname != writer || logined == false) {
        ...
        connection.query(
          `delete from writing_list where title = "${title}"`,
          function (error, results, fields) {
            res.redirect("/");
          }
        );
...
app.post("/signin", (req, res) => {
  ...
  var pw = req.body.pw;
  connection.query(
    `select salt,pw,nickname from user_list where id="${id}"`,
...
app.post("/signup", (req, res) => {
  ...
  connection.query(
    `select * from user_list where id=${id} or nickname=${nickname}`,
    function (error, results) {
        ...
        crypto.randomBytes(10, (err, buf) => {
          crypto.pbkdf2(
            ...
            (err, key) => {
              ...
              connection.query(
                `INSERT INTO user_list(id, pw, salt, nickname) VALUES ("${id}", "${pw}", "${salt}","${nickname}")`,
                function (error, results, fields) {
                  if (error) {
                    throw error;
                  } else {
